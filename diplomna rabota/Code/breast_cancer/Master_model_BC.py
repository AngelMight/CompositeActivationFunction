...
y_pd=np.array(df['class'])
y_pd = np.where(y_pd==4, 1, y_pd)
y_pd = np.where(y_pd==2, 0, y_pd)

X_pd=np.array(df.iloc[:,:-1])
...
X = (X.reshape(X.shape[0], -1).astype(np.float32) - 10) / 10
...
model.train(X, y, validation_data=(X_test, y_test),
            epochs=EPOCHS, print_every=50)

#Evaluate the model Chap 20
...
'''
#Load the model
model = Model.load('breast_cancer.model')
'''
...
#Set the parameters
# Set model with parameters instead of training it, page 608
model.set_parameters(parameters)

#Save the parameters in a file in binary code
model.save_parameters('breast_cancer.parms')

# Set model with parameters instead of training it
# Load + set parameters
model.load_parameters('breast_cancer.parms')
'''